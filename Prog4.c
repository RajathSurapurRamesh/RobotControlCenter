...
// Initialize USART2 and USART2 interupts.
  ...
  BSP_IntEn(BSP_INT_ID_USART2);           // Enable IRQ38.
    InitSerIO();                         // Initialize the USART2 I/O Driver.
  InitMemMgr();                          //intializing the memeory manager
  CreateParserTask();                    //create parser task 
  CreateFramerTask();                    //create framer task
  CreateRobotManagerTask();              //create robotmanager task
